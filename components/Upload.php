...
class Upload {
    ...
    function __construct($BaseDir = null) {
        self::$BaseDir = ( (string) $BaseDir ? $BaseDir : \Yii::getAlias('@webroot') . '/uploads/');
     ...
     */
    public function Image($Image, $Name = null, $Width = null, $Folder = null) {
        $this->File = $Image;
     ...
     * @param FILES $File = Enviar envelope de $_FILES (PDF, DOCX, EXCEL, POWERPOINT, ZIP, TXT, AUTOCAD, ACCESS)
     * @param STRING $Name = Nome do arquivo ( ou do artigo )
     * @param STRING $Folder = Pasta personalizada
     * @param STRING $MaxFileSize = Tamanho máximo do arquivo (4mb)
     */
    public function File($File, $Name = null, $Folder = null, $MaxFileSize = null) {
        $this->File = $File;
        ...
        $FileAccept = [
            ...
            // Excel
            'text/csv', //csv
            'application/vnd.ms-excel', //xls , xlt, xla, xlw, xlm, xll, xlc
...
//        $this->CreateFolder("{$Folder}/{$y}/{$m}/");
    ...
    private function MoveFile() {
        if (move_uploaded_file($this->File->tempName, self::$BaseDir . $this->Send . $this->Name)):
            $this->Result = $this->Send . $this->Name;
            $this->Error = null;
        else:
            $this->Result = false;
